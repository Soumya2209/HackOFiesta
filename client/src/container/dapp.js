...
class Dapp extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (networkData) {
      ...
      this.setState({ court });
      this.setState({ GAS: 500000, GAS_PRICE: "20000000000", GAS_LIMIT: "5000000" });
  ...
  async addEncryptedKey(isLawyer, ljId, caseId, key) {
    const { account, court, GAS, GAS_PRICE, GAS_LIMIT } = this.state;
    await court.methods
      .addEncryptedKey(isLawyer, ljId, caseId, key)
      .send({ from: account, gas: GAS, gasPrice: GAS_PRICE, gasLimit: GAS_LIMIT })
      .then(r => {
        console.log(r);
      })
      .catch(e => {
